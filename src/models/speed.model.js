...
Speed.getAllSpeedM = function (result) {
  pool.query(
    "select * from speed where status=1  order by speed_id desc",
    function (err, res) {
      if (err) {
        console.log(err);
        result(err, null);
...
Speed.getSpeedByIdM = function (speed_id, result) {
  pool.query(
    `select * from speed Where status=1 and speed_id=?`,
    [speed_id],
    function (err, res) {
      if (err) {
        console.log(err);
...
Speed.updateSpeedM = function (speed_id, speed, result) {
  pool.query(
    "update speed SET speed_name=?, modifiedById=?, modificationDate=? where speed_id=?",
    [
      speed.speed_name,
      //   speed.speed,
      speed.modifiedById,
      speed.modificationDate,
      speed_id,
    ],
    function (err, res) {
      if (err) {
