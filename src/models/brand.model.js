...
//  Add Brand
Brand.insertBrandM = function (brand, result) {       
    ...
    Brand.updateBrandM = function(id,brand,result) {
        ...
        pool.query(`select * from brand where brand_id=${id}`,function(err, data){
            if(err){
                console.log(err);
                result(err, null);
            }
                ...
                if(data.length>0)
                {
                    if(brand.image !== undefined){
                        update_query = `update brand SET brand_name=?, image=?, modifiedById=?, 
                        modificationDate=? where brand_id=?`;
    
                        value = [brand.brand_name, brand.image,brand.modifiedById, brand.modificationDate, id];
                    }
                    else{
                        update_query = `update brand SET brand_name=?, modifiedById=?, 
                        modificationDate=? where brand_id=?`;
    
                        value = [brand.brand_name,brand.modifiedById, brand.modificationDate, id];
                    }
  ...
  Brand.getAllBrand= function (result) {       
    pool.query("select * from brand where status=1  order by brand_id desc", function (err, res) {
            if(err) {
                console.log(err);
                result(err, null);
            }
...
Brand.getBrandByIdM = function (brand_id, result) {       
    pool.query(`select * from brand Where status=1 and brand_id=?`,[brand_id], function (err, res) {
            if(err) {
                console.log(err);
                result(err, null);
            }
